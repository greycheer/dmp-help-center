...
const adminImages = [
  ...
  { token: "Nnr9bUJJKoyhf3xUToAcjk9cnVd", name: "admin-cloud-order-01" },
  { token: "W37Db51sbojU7XxnR3EcgQ6Un0b", name: "admin-cloud-order-02" },
  { token: "CVMWb1p8moQR75xda0pcKt03nog", name: "admin-cloud-order-03" },
...
function downloadImage(token, filename) {
  return new Promise((resolve) => {
    ...
    // For now, create placeholder files since we need tenant token for actual download
    fs.writeFileSync(filePath, `Image: ${token}`);
    console.log(`Created placeholder: ${filename}.png`);
    resolve();
  });
}
...
async function main() {
  ...
  for (const img of adminImages) {
    await downloadImage(img.token, img.name);
  }
  ...
  console.log(`\nTotal: ${adminImages.length + developerImages.length} images`);
}
